...
public class AddCalendarController {
    ...
    public void handleSave(ActionEvent event) {
        ...
        Calendars newCalendar = new Calendars(0,name);

        if (!name.isEmpty()) {
            DBHelper dbHelper = new DBHelper();
            dbHelper.saveCalendar(newCalendar); // Υποθέτοντας ότι έχεις αυτή τη μέθοδο στον DBHelper

            // Κλείσιμο του διαλόγου
            Dialog<ButtonType> dialog = new Dialog<>();
            dialog.setContentText("Το ημερολόγιο προστέθηκε επιτυχώς!");
